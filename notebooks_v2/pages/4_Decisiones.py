...
with tab1:
    ...
    # ── Materias detonantes vs acumuladoras ───
    st.markdown("### Materias detonantes vs. acumuladoras")
    st.caption(
        "**Detonante:** materia que concentra el mayor número de estudiantes bajo 4.0 en T2 — "
        "foco de intervención inmediata. "
        "**Acumuladora:** materia cuyo promedio acumulado (T1×0.30 + T2×0.30 + T3×0.40) "
        "amenaza la aprobación del año."
    )

    avail = [s for s in SUBJECTS if f"{s}_T2" in df_g.columns]
    det_rows = []
    for s in avail:
        n_bajo_t2  = (df_g[f"{s}_T2"] < 4.0).sum()
        pct_bajo   = round(n_bajo_t2 / len(df_g) * 100, 1) if len(df_g) > 0 else 0
        avg_t2     = df_g[f"{s}_T2"].mean()

        # Promedio acumulado si T3 disponible
        if f"{s}_T3" in df_g.columns:
        ...
        det_rows.append({
            ...
            "Bajo 4.0 en T2":   n_bajo_t2,
            "% grupo":          pct_bajo,
            "Prom. T2":         round(avg_t2, 2) if not np.isnan(avg_t2) else None,
            "Acum. en riesgo":  n_acum_riesgo,
            "_s":               s
        })

    df_det = pd.DataFrame(det_rows).sort_values("Bajo 4.0 en T2", ascending=False)

    fig_det = go.Figure()
    fig_det.add_trace(go.Bar(
        name="Bajo 4.0 en T2 (detonante)",
        x=df_det["Materia"],
        y=df_det["Bajo 4.0 en T2"],
        marker_color="#e74c3c",
        text=df_det["Bajo 4.0 en T2"],
    ...
    df_det_show = df_det.drop(columns=["_s"]).rename(columns={
        "Bajo 4.0 en T2": "Bajo 4.0 T2",
        "Acum. en riesgo": "Acum. riesgo"
    })
    ...
    st.markdown("### Comparativa por sección")
    st.caption("¿Hay diferencias sistemáticas entre secciones del mismo grado?")

    if grado_sel == "Todos":
        st.info("Selecciona un grado en el sidebar para ver la comparativa por sección.")
    else:
        ...
        if len(secciones) < 2:
            ...
            comp_rows = []
            for s in avail:
                row_data = {"Materia": SUBJECT_LABELS[s]}
                for sec in secciones:
                    df_sec = df_g[df_g["seccion"] == sec]
                    avg = df_sec[f"{s}_T2"].mean()
                    row_data[f"Secc. {sec} — Prom T2"] = round(avg, 2) if not np.isnan(avg) else None
            ...
            for i, sec in enumerate(secciones):
                ...
                if col_name in df_comp.columns:
                    fig_comp.add_trace(go.Bar(
                        ...
                        text=df_comp[col_name].round(2),
                        textposition="outside",
                        textfont=dict(size=9)
                    ))
            fig_comp.add_hline(y=4.0, line_dash="dash", line_color="#e74c3c",
                               annotation_text="Mínimo 4.0", annotation_font_color="#e74c3c")
            fig_comp.update_layout(
                barmode="group", height=380,
                xaxis=dict(tickangle=-30, gridcolor="#f0f0f0"),
                yaxis=dict(title="Promedio T2", range=[0, 7.8], gridcolor="#f0f0f0"),
                legend=dict(orientation="h", yanchor="bottom", y=1.02),
                plot_bgcolor="white", paper_bgcolor="white",
                margin=dict(l=40, r=20, t=50, b=90)
            )
...
with tab2:
    ...
    df_gain["interv_pct"]   = df_gain["rank"] / len(df_gain) * 100
    ...
    fig_gain.update_layout(
        ...
        yaxis=dict(title="% estudiantes en riesgo capturados", range=[0, 105],
                   gridcolor="#f0f0f0"),
        legend=dict(orientation="h", yanchor="bottom", y=1.02),
        plot_bgcolor="white", paper_bgcolor="white",
    ...
    # Cuántos en riesgo captura con esa capacidad
    ...
    st.markdown(f"**Top {capacidad} estudiantes a intervenir esta semana — ordenados por riesgo**")
